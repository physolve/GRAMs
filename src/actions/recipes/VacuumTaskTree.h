...
//   К192 → "SL1"  (второй тракт: выход, профиль v_gasOutlet)
//   К179 → "SL2"  (клапан турбомолекулярного насоса, профиль v_gasVacuum)
//
...
// ВНИМАНИЕ: К192 — это SL1, К179 — это SL2, не наоборот. На турбомолекулярный
// насос идёт SL2 (К179): в профиле этот же канал назван v_gasVacuum, а SL1 —
// v_gasOutlet, выход второго тракта. Перепутанное соответствие означает команду
// не тому DO-каналу — то есть турбоклапан не открывается, а вместо него
// открывается выход тракта.
inline const QString K192 = QStringLiteral("SL1");
inline const QString K179 = QStringLiteral("SL2");
...
//
// targetPa — цель этапа откачки. Проверка dP/dt осмысленна ТОЛЬКО пока давление
// выше цели: если система уже на цели или ниже (цель 50 Па, а в тракте 30 Па),
// откачивать нечего, dP/dt близка к нулю или отрицательна по физике процесса,
// и «мала» здесь означало бы не отказ насоса, а достигнутую цель. Поэтому при
// достоверном показании ≤ targetPa задача завершается успехом сразу, не открывая
// окно и не поднимая оператора. Значение по умолчанию (−∞) выключает это
// правило: проверка идёт всегда, как раньше.
...
public:
    ...
    // Цель этапа: при показании ≤ targetPa проверка не проводится (см. выше).
    double targetPa     = -std::numeric_limits<double>::infinity();
...
private:
    void beginWindow();
    bool atTarget() const;             // достоверное показание ≤ targetPa
...
struct VacuumRunState {
    ...
    bool k192Open = false;   // SL1 — выход второго тракта (Ф3)
    // Ф3 сама закрывает К179 в конце фазы; k179Open отражает факт открытия
    // турбоклапана в любом месте рецепта, k179TractOpen — только «открыт из Ф3».
    bool k179TractOpen = false;
    bool k176Open = false;   // AR6 — форвакуумный насос
    bool k179Open = false;   // SL2 — турбомолекулярный насос (12.2)
    // Переход 12.2 состоялся и не откатывался. Это НЕ то же самое, что
    // k179Open: турбоклапан закрывается по завершении этапа (REQ-058), но
    // факт достигнутого турбо-режима сохраняется до конца повтора и решает,
    // каким насосом идёт финальная откачка камеры 11.10. Откат на форвакуум
    // (turboFallback) сбрасывает флаг — только после него 11.10 снова
    // открывает К176.
    bool turboReached = false;
    bool k178Open = false;   // AR5 — магистраль
    bool k151Open = false;   // R3  — линия E/F
    // Клапаны тракта, открытые текущим этапом по набору из конфигурации
    // (11.7б/11.8/11.9–11.10). Порядок — порядок открытия; закрытие идёт
    // обратным обходом, поэтому насос отсекается от объёмов раньше, чем
    // объёмы разъединяются между собой.
    QStringList tractOpen;
...
enum class VacuumNode {
    ...
    GeneralPumping,  // 11.7б: общая откачка (К171/К173/К151/C → К178 → насос)
    LeakTest,        // 11.8: проверка герметичности (К171/К173, выдержка)
    FinalPrep,       // 11.9: подготовка тракта финальной откачки камеры
    FinalPumping,    // 11.10: финальная откачка камеры (EvacTime + targetVAC)
    FinalClose,      // 11.11: финальное закрытие и запись итогов
    ContinuousPumping // финал: тракт оставлен открытым под откачку (опция)
};

// ─── Канал проверки герметичности (11.8, REQ-060) ─────────────────────────────
//
// Единицу задаёт шов read, как и везде в Reading: для ДД312 это бары, и maxRise
// задаётся в тех же единицах. Пустой список каналов означает, что порог
// dP_leak_max в конфигурации не задан — этап тогда обязан быть Skipped с
// причиной, а не молча «пройден» (REQ-062: «не выполнялся» ≠ «пройдено»).

struct LeakChannel {
    QString                  name;        // «ДД312» — для журнала и предупреждения
    std::function<Reading()> read;        // показание канала
    double                   maxRise = 0.0;  // допустимый прирост за выдержку
};

// ─── Бюджет времени прогона ───────────────────────────────────────────────────
//
// СОГЛАСОВАННАЯ ДИВЕРГЕНЦИЯ ОТ ТЗ v5 (REQ-032/033/068/069).
//
// ТЗ делает EvacTime приоритетным условием: откачка идёт до его истечения даже
// при достигнутом давлении, а при недостигнутом продлевается до
// maxAdditionalEvacTime. Здесь время работает иначе — как ЖЁСТКИЙ ПОТОЛОК
// сверху: T_total берётся из строки RunTable (поле max_time, секунды) и
// ограничивает ВЕСЬ прогон режима, от старта Ф1. maxAdditionalEvacTime не
// реализован вовсе: продление отменено бюджетом.
//
// Практический смысл: оператор планирует эксперимент по строкам RunTable, и
// строка, которая может идти дольше объявленного, ломает весь план прогона,
// а не только себя. Расхождение согласовано и подлежит внесению в следующую
// редакцию ТЗ (см. CLAUDE.md и docs/regimes/vacuum.md).
//
// Счётчик ведётся в ТИКАХ рецепта, а не по часам: тик равен секунде в бою и
// миллисекунде в тестах, поэтому wall-clock дал бы в тестах бюджет в тысячу
// раз меньше заявленного. Все ожидания рецепта идут через PausableTicker,
// поэтому один тикающий счётчик покрывает их все.
struct RunBudget {
    int  totalSec = 0;     // T_total; 0 = без ограничения (прежнее поведение)
    int  usedSec  = 0;     // потрачено с начала execution-фазы повтора
    // Причина «бюджет исчерпан» пишется один раз за повтор: дальше её увидит
    // каждый следующий этап, и дублировать её в журнале незачем.
    bool reported = false;

    bool limited()   const { return totalSec > 0; }
    bool exhausted() const { return limited() && usedSec >= totalSec; }
    int  remaining() const {
        return limited() ? qMax(0, totalSec - usedSec)
                         : std::numeric_limits<int>::max();
    }
    // Обрезать длительность ожидания остатком бюджета. Ожидание, выходящее за
    // потолок, не «почти укладывается» — оно его нарушает.
    int clamp(int sec) const { return limited() ? qMin(sec, remaining()) : sec; }
};
...
struct VacuumTreeContext {
    ...
    // Форвакуумные этапы 11.5–11.7 ВСЕГДА выполняются и всегда предшествуют
    // любому турбо-этапу (REQ-047/050/053/056/077). Выключателя нет ни здесь,
    // ни в UI: турбонасос, запущенный на неоткачанном тракте, — это его порча,
    // и такое решение не должно быть доступно с рабочего экрана.
    ...
    // SL1/К192 (выход второго тракта; атмосфера, если камеры нет). К176 и К179
    ...
    double turboSwitchPressurePa = 50.0;   // гейт перехода по ДВ301 (REQ-078/080)
    int    turboSwitchHoldSec    = 60;     // непрерывное удержание гейта (REQ-080)
    double turboReturnPressurePa = 150.0;  // порог отката на форвакуум (REQ-083)
    int    turboTimeoutSec       = 600;    // лимит набора гейта целиком
    // Гистерезис 50 Па на вход / 150 Па на выход — намеренный: при равных
    ...
    double pumpMinDropPa      = 0.25;  // мин. падение давления за окно
    OperatorBus* operatorBus = nullptr;

    // ── Этапы 11.7б / 11.8 / 11.9–11.11 ───────────────────────────────────────
    //
    // Наборы клапанов приходят из profile/GRAMsPfp.json → vacuumTract, а не из
    // кода: REQ-055 прямо требует хранить точный набор в конфигурации. Насосные
    // клапаны (К176/К179) в списки НЕ входят — ими распоряжается
    // PumpDownProcedure, и путать тракт с насосом нельзя. Пустой список =
    // этап не сконфигурирован ⇒ пропускается с явной причиной, а не молча.
    QStringList generalPumpingValves;   // 11.7б: К171, К173, К151 (REQ-055)
    QStringList leakTestValves;         // 11.8:  К171, К173      (REQ-059)
    QStringList finalPumpingValves;     // 11.9/11.10: К173, К151, К171, К178 (REQ-064/066)

    int    testEvacTimeSec     = 300;   // 11.7б: откачка после перехода на К179 (REQ-057)
    int    leakTestDurationSec = 60;    // 11.8:  выдержка проверки (REQ-060)
    // 11.8: каналы и пороги. Пусто ⇒ этап Skipped с причиной (см. LeakChannel).
    QList<LeakChannel> leakChannels;

    // Бюджет времени всего прогона режима (см. RunBudget выше). Приходит из
    // строки RunTable через RegimeWorkerConfig::maxTimeSec. 0 = без ограничения.
    int totalBudgetSec = 0;
    // Живой счётчик бюджета. Создаётся в buildVacuumRecipe и общий для всего
    // дерева: копии контекста в лямбдах делят один shared_ptr.
    std::shared_ptr<RunBudget> budget;

    // 11.10: цель и длительность финальной откачки камеры (REQ-029/032/065–069).
    // Датчик выбирается по области: targetVacuumPa ≥ turboSwitchPressurePa —
    // форвакуумная область, контроль по ДВ301 и переход на К179 не обязателен
    // (REQ-067); ниже порога — турбо-область, контроль по ДВ302.
    double targetVacuumPa = 0.0133;     // targetVAC, по умолчанию 1e-4 Торр (REQ-036)
    int    evacTimeSec    = 1800;       // EvacTime, по умолчанию 30 мин (REQ-036)

    // Непрерывная откачка: после успешного завершения ВСЕХ повторов оставить
    // рабочий тракт открытым на ТУРБОНАСОСЕ (К151 → К178 → К179, форвакуумный
    // К176 закрыт), чтобы откачка продолжалась без автоматического закрытия.
    // К118 (атмосфера), баллоны C и К192 не открываются. При отмене/ошибке
    // этап пропускается — рецепт закрывает всё, как раньше.
    bool   continuousPumping = false;
    // Живое значение той же опции (nullable). Оператор вправе передумать, пока
    // режим идёт, поэтому хвост спрашивает состояние тумблера в момент своего
    // выполнения, а не берёт снимок, сделанный на «Старт». Если шов не задан,
    // используется поле continuousPumping выше — так работают тесты.
    std::function<bool()> continuousPumpingLive;
    ...
    //   node    — TurboGate / TurboPumping / FinalPumping
    //   p301    — ДВ301 с качеством, p302 — ДВ302 с качеством
    //   heldSec — набранное непрерывное удержание гейта (0 на откачке)
    //   limitSec — предел ИМЕННО ЭТОГО этапа: удержание гейта для TurboGate,
    //     фактическая длительность откачки для TurboPumping/FinalPumping.
    //     Шов обязан назвать его сам: у гейта и у откачки пределы разные и
    //     ни один из них не выводится из порогов профиля. Пока предел
    //     подставлял монитор, полоса турбо-откачки шла к turboTimeoutSec
    //     (10 мин потолка гейта) и этап заканчивался на середине — на
    //     testEvacTimeSec (5 мин), по которым он и работал.
    std::function<void(VacuumNode node, Reading p301, Reading p302,
                       int heldSec, int elapsedSec, int limitSec)> onTurboProgress;
    ...
    // Расход бюджета времени (nullable): сколько прошло с начала прогона и
    // сколько осталось. UI обязан показывать суммарное время прогона, а не
    // только прогресс текущего этапа — иначе «сколько ещё» узнать неоткуда.
    std::function<void(int elapsedSec, int remainingSec)> onBudget;
    ...
    // Этап не выполнялся и ПОЧЕМУ (nullable). Отдельно от onFailure намеренно:
    // пропуск — не отказ, но и не успех, и в журнале это обязано различаться,
    // иначе «проверка герметичности не проводилась» прочитается как «пройдена».
    std::function<void(VacuumNode, const QString& reason)> onStageSkipped;
    // Предупреждение без остановки режима (nullable). REQ-062: LEAK_DETECTED
    // в режиме «Вакуум» — предупреждение, режим продолжается.
    std::function<void(const QString& message)>      onWarning;
