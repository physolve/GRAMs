...
Правила давления в `Security` используют датчики квартилей, которые
`Grams::softEvent` передаёт каждый такт: DD311/DD331 всегда, DD312/DD332 — только
при открытом S4/R4. Давление резервуара — максимум из достоверных. Открыть S4
или R4 можно только при достоверном давлении ниже 1,6 бар. До первого такта
после запуска и при недостоверном показании открытие отклоняется с причиной
`pressure_invalid`, в зоне 1,6–1,8 бар — `pressure_range`.

**S4 (К171) и R4 (К173) Security закрывает сам** на каждом такте `softEvent`,
идёт режим или нет, той же записью в порт, что и обычная команда. Поэтому эхо,
`sim.status.valves`, мнемосхема и Lumber показывают одно и то же. Причины:

- `pressure_range_autoclose` — давление резервуара выше 1,8 бар;
- `transfer_equilibrium` — давление равновесия через К151/К153/К155 выше 1,8 бар;
- `unauthorized_open` — клапан открыт не оператором и не идущим режимом: при
  старте, по readback, после конца режима.

Открывать клапаны Security не умеет. Демо ограничивает канал шкалой датчика:
DD312/DD332 выше 2,0 бар не поднимаются. Отказ токовой петли, то есть
недостоверное показание узкодиапазонного датчика, демо не воспроизводит — это
проверяется тестами.
...
| `grams.security` | отказ с кодом (`interlock` с открытыми клапанами, `pressure_range`, `pressure_invalid` с пояснением); «Security закрыл S4: pressure_range_autoclose (storageQuar DD312 = …)» с источником и местом обнаружения; прогноз равновесия (debug); предупреждения о недостоверном давлении; нарушения `checkPressure` |
...
| `SimIntegrationTests` · `SimSecurityRange` | Профиль `security_range`: S4/R4, открытые щелчком на 1,01 бар, закрываются не позже такта после 1,8 бар; при спуске сами не открываются; гистерезис 1,6–1,8. «Тест клапанов» и «Стоп» — через настоящий `RegimeTaskTree`. Открыт на плате при старте, найден readback. Прогноз равновесия через К15x (`security_transfer`, обе стороны). Недостоверный DD312 не прячет давление по DD311; запертый DD312 не мешает открытию. Реакция `RegimeWorkerBase` и настоящего `VacuumRegimeWorker`, строка оператору |
...
- **Порог `cond_gasRelease = 55` бар выше диапазона DD311 (0…50 бар).** Порог нужно пересмотреть в профиле.
- **Правило AR4 в приложении не действует (найдено 23.09.2026, не исправлено).** `Grams::initSafeModule` берёт клапан сброса из `storageQuar.v_gasRelease`, а в `GRAMsPfp.json` он лежит в `addRemoveQuar`. Тестовые риги `SimValves`/`SimVacuumIntegration` брали его из `addRemoveQuar`, поэтому тест «AR4 без давления отклонён» проверял не то, что работает в GRAMs. Зеркало `soft_event_mirror.h` повторяет приложение как есть.
- **S4/R4, 23.09.2026.** Security закрывает клапаны диапазона сам: по давлению, по прогнозу равновесия, если они открыты без права. Режимы получают `security_violation`, оператор видит строку в `footer`. Тесты `SimSecurityRange` (18) воспроизводили дефекты до исправления. Отчёт — `Notes/Day13/2026-09-23-security-S4-R4-итоги.md`.
  - **Проверено вживую до исправления:** `GRAMs --sim`, профиль `grams_security_range`, щелчок S4/R4 на 1,01 бар, рост до 1,95 бар. К171/К173 остаются открытыми, строк `grams.security` у перехода 1,8 бар нет.
  - **Проверено вживую после исправления** (этап 5.5, шаги 1–3). Щелчок S4/R4 на 1,01 бар — открыты, источник `manual`. На росте Security закрыл оба клапана на первом такте, где показание стало больше 1,8: DD312 = DD332 = 1,80168 бар, `pressure_range_autoclose`, `tick`; `sim.status` в 23:46:44 — 1,794 бар, клапаны открыты, в 23:46:45 — 1,809 бар, закрыты. В `footer` — красное сообщение. На 1,95 бар щелчок S4 — отказ `pressure_range` по DD311. Лог — `data/sim/day13-s4r4-fixed.log`.
  - **Не проверено вживую** (покрыто тестами `SimSecurityRange`): шаги 4–8 — гистерезис 1,7, спуск до 1,4, «Тест клапанов» [S4, R3] при росте, «Стоп» во время выдержки, перепуск `security_transfer`. GRAMs остановлен системой из-за нехватки памяти, снимок `gramstate_sim` при этом не записан. Стенд (этап 8.3) — не проверен.
